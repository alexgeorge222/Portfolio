...
public class EdgeWeightedDigraph {
	...
	private Bag<DirectedEdge>[] miles; 
	private Bag<DirectedEdge>[] price;// adj[v] = adjacency list for vertex v
	...
	public EdgeWeightedDigraph(int V) {
		...
		miles = (Bag<DirectedEdge>[]) new Bag[V];
		price = (Bag<DirectedEdge>[]) new Bag[V];
		for (int v = 0; v < V; v++){

			miles[v] = new Bag<DirectedEdge>();
			price[v] = new Bag<DirectedEdge>();
		}
	}
	...
	public EdgeWeightedDigraph(In in) {
		...
		while (in.hasNextLine()) {
			...
			addMEdge(new DirectedEdge(v, w, miles));
			addPEdge(new DirectedEdge(v, w, price));
		}
	 ...
	 */
	public void addMEdge(DirectedEdge e) {
		int v = e.from();
		int w = e.to();
		validateVertex(v);
		validateVertex(w);
	...
	public void addPEdge(DirectedEdge e) {
		int v = e.from();
		int w = e.to();
		validateVertex(v);
		validateVertex(w);
	...
	public Iterable<DirectedEdge> adj2(int v) {
		validateVertex(v);
		return price[v];
	}
	...
	public Iterable<DirectedEdge> pedges() {
		Bag<DirectedEdge> list = new Bag<DirectedEdge>();
		for (int v = 0; v < V; v++) {
			for (DirectedEdge e : adj2(v)) {
				list.add(e);
			}
		}
		return list;
