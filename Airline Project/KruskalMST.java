...
public class KruskalMST {
    ...
    private double mWeight;                        // weight of MST
    private Queue<Edge> mMst = new Queue<Edge>();  // edges in MST
    ...
    public KruskalMST(EdgeWeightedGraph G) {
        ...
        UF uf = new UF(G.V());
        while (!pq.isEmpty() && mMst.size() < G.V() - 1) {
            Edge e = pq.delMin();
            int v = e.either();
            int w = e.other(v);
            if (!uf.connected(v, w)) { // v-w does not create a cycle
                uf.union(v, w);  // merge v and w components
                mMst.enqueue(e);  // add edge e to mst
                mWeight += e.weight();
            }
        }
